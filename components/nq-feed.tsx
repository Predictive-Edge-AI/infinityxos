...
export function NQFeed() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const currentPrice = 18300 + Math.random() * 400
      const change = Math.random() > 0.6 ? Math.random() * 350 : -Math.random() * 200
      const percentChange = (change / currentPrice) * 100
      const prediction = currentPrice + (Math.random() > 0.6 ? Math.random() * 500 : -Math.random() * 250)
    ...
    <div className="space-y-3">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-2">
          <div className="h-8 w-8 rounded-full bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center">
            <span className="text-xs font-bold text-white">NQ</span>
          </div>
          <div>
            <h3 className="text-sm font-medium text-white">Nasdaq 100 Futures</h3>
            <p className="text-xs text-white/60">E-mini Futures</p>
          </div>
        </div>
      ...
      <Card className="bg-black/40 border-white/5 p-3">
        <div className="flex justify-between items-center mb-2">
          <h4 className="text-xs font-medium text-white">AI Prediction (24h)</h4>
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center gap-2">
            <div className="h-10 w-10 rounded-full bg-gradient-to-br from-blue-500/20 to-indigo-600/20 border border-white/10 flex items-center justify-center">
              <span className="text-xs font-bold text-white">{data.confidence.toFixed(0)}%</span>
            </div>
